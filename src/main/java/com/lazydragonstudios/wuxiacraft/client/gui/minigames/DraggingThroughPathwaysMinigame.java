...
public class DraggingThroughPathwaysMinigame implements Minigame {
	...
	public void render(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTick) {
		...
		guiGraphics.blit(MINIGAME_TEXTURE, imageX, imageY, 148, 135, 0, 0, 148, 135, 512, 512); //the white body
		//fill = 60 * cult_base/max_cult_base
		int barFill = BigDecimal.valueOf(135).multiply(
				cultivation.getStat(System.ESSENCE, PlayerSystemStat.CULTIVATION_BASE)
						.divide(cultivation.getStat(System.ESSENCE, PlayerSystemStat.MAX_CULTIVATION_BASE), RoundingMode.HALF_UP)).intValue();
		guiGraphics.blit(MINIGAME_TEXTURE, imageX, imageY + 135 - barFill, 148, barFill, 156, 135 - barFill, 148, barFill, 512, 512); //the green body fill
		guiGraphics.blit(MINIGAME_TEXTURE, imageX + 2, imageY + 21, 144, 111, 0, 135, 144, 111, 512, 512); //the pathways
		guiGraphics.blit(MINIGAME_TEXTURE, imageX + dantian.x - 4, imageY + dantian.y - 4, 8, 8, dantian.width, dantian.height, 8, 8, 512, 512);
		if (this.selectedStrand != null) {
			if (this.chosenPathway == -1) {
				for (int i = 0; i < 4; i++) {
					Point point = pathways[i][0];
					guiGraphics.blit(MINIGAME_TEXTURE, imageX + point.x - 3, imageY + point.y - 3, 5, 148, 13, 5, 5, 512, 512);
				}
	...
	private static class Strand {
		...
		void render(GuiGraphics guiGraphics) {
			...
			guiGraphics.blit(MINIGAME_TEXTURE, -3, -3, 5, 5, 148, 0, 5, 5, 512, 512);
