...
public class BodyMinigame implements Minigame {
	...
	public boolean onMouseRelease(double x, double y, int button) {
		if (this.selectedStrand != null) {
			WuxiaPacketHandler.INSTANCE.sendToServer(new MeditateMessage(System.BODY, false));
			var player = Minecraft.getInstance().player;
			if (player == null) return false;
			var cultivation = Cultivation.get(player);
			var bodyData = cultivation.getSystemData(System.BODY);
			bodyData.getStage().cultivationFailure(player);
	...
	public void render(PoseStack poseStack, int mouseX, int mouseY, float partialTick) {
		...
		int barFill = BigDecimal.valueOf(135).multiply(
				bodyData.getStat(PlayerSystemStat.CULTIVATION_BASE)
						.divide(bodyData.getStat(PlayerSystemStat.MAX_CULTIVATION_BASE), RoundingMode.HALF_UP)).intValue();
	...
	public void tick() {
		...
		var cultivation = Cultivation.get(player);
		var bodyData = cultivation.getSystemData(System.BODY);
		var strandCount = bodyData.getStat(PlayerSystemStat.ENERGY).divide(new BigDecimal("4"), RoundingMode.HALF_UP).intValue();
			...
			if (selectedStrand == strand) {
				if (chosenPathway == -1) {
					...
					if (this.currentPoint >= pathways[this.chosenPathway].length) {
						...
						WuxiaPacketHandler.INSTANCE.sendToServer(new MeditateMessage(System.BODY, true));
						bodyData.getStage().cultivate(player);
