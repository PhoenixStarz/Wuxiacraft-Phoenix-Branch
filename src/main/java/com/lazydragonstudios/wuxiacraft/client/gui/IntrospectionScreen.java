...
public class IntrospectionScreen extends AbstractContainerScreen<IntrospectionMenu> {
	...
	protected void init() {
		...
		tabs.put("sects", new SectLessTab("sects", new Point(32,68)));
		tabsOrder.add("sects");
	...
	public boolean mouseDragged(double mouseX, double mouseY, int button, double mouseDeltaX, double mouseDeltaY) {
		...
		for (GuiEventListener guieventlistener : this.children()) {
			if(MathUtil.inBounds(mouseX, mouseY,
					guieventlistener.getRectangle().left(), guieventlistener.getRectangle().top(),
					guieventlistener.getRectangle().width(), guieventlistener.getRectangle().height())) {
	...
	protected void renderBg(GuiGraphics guiGraphics, float partialTicks, int mouseX, int mouseY) {
		...
		for (var tab : this.tabsOrder) {
			...
			guiGraphics.blit(INTROSPECTION_GUI,
		...
		for (var system : System.values()) {
			...
			BigDecimal maxEnergy = cultivation.getStat(system, PlayerSystemStat.MAX_ENERGY);
			if (maxEnergy.compareTo(BigDecimal.ZERO) == 0) {
				maxEnergy = BigDecimal.ONE;
			}
			int barFill = new BigDecimal((stretchedSpace + 10) + ".000").multiply(cultivation.getStat(system, PlayerSystemStat.ENERGY)).divide(maxEnergy, RoundingMode.HALF_UP).intValue();
			guiGraphics.blit(INTROSPECTION_GUI,
		...
		//render the tab
		this.tabs.get(this.selectedTab).renderBg(guiGraphics, mouseX, mouseY);
	}
	...
	public void getSectTab() {
		var player = Minecraft.getInstance().player;
		if(player == null) return;
		var cultivation = Cultivation.get(player);
		var sectId = cultivation.getSectId();
		if(sectId == null && !( tabs.get("sects") instanceof SectLessTab))
			this.tabs.put("sects", new SectLessTab("sects", new Point(32,68)));
		else if(sectId != null && !( tabs.get("sects") instanceof InASectTab))
			this.tabs.put("sects", new InASectTab("sects", new Point(32,68)));
	}
