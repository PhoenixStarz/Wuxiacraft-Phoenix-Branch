...
public class ClosingTheCircleMinigame implements Minigame {
	...
	public void render(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTick) {
		...
		int barFill = BigDecimal.valueOf(60).multiply(
						cultivation.getStat(System.ESSENCE, PlayerSystemStat.CULTIVATION_BASE)
								.divide(cultivation.getStat(System.ESSENCE, PlayerSystemStat.MAX_CULTIVATION_BASE), RoundingMode.HALF_UP))
	...
	private static class Strand {
		...
		void tick(double outerCircleRadius) {
			...
			this.x += movX;
			this.y += movY;
			if (ClosingTheCircleMinigame.isOutsideCircle(this.x, this.y, CENTER_X, CENTER_Y, outerCircleRadius)) {
				this.movX *= -1;
				this.movY *= -1;
			}
		}
