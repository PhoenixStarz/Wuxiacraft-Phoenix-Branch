...
public class DivineMinigame implements Minigame {
	...
	public void render(PoseStack poseStack, int mouseX, int mouseY, float partialTick) {
		...
		int barFill = BigDecimal.valueOf(60).multiply(
						divineData.getStat(PlayerSystemStat.CULTIVATION_BASE)
								.divide(divineData.getStat(PlayerSystemStat.MAX_CULTIVATION_BASE), RoundingMode.HALF_UP))
	...
	public void tick() {
		...
		var cultivation = Cultivation.get(player);
		var divineData = cultivation.getSystemData(System.DIVINE);
		int energy = divineData.getStat(PlayerSystemStat.ENERGY).intValue();
