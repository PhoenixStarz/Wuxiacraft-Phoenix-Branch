...
public class TechniqueTab extends IntrospectionTab {
	...
	public void init(IntrospectionScreen screen) {
		...
		composerPanel = new WuxiaScrollPanel(36 + 116, 36, stretchedSpace, scaledHeight - 36, Component.empty());
		composerPanel.setOverflow(WuxiaScrollPanel.OverflowType.HIDDEN);

		compileBtn = new WuxiaButton(scaledWidth - 190, scaledHeight - 50, 180, 20, Component.literal("Compile"), () -> {
			try {
		...
		saveBtn = new WuxiaButton(scaledWidth - 190, scaledHeight - 25, 180, 20, Component.literal("Save"), () -> {
			if (this.isCompiled) {
		...
		var radius = cultivation.getStat(system, PlayerSystemStat.ADDITIONAL_GRID_RADIUS).intValue();
...
/* 	private BiConsumer<Double, Double> onAspectRelease() {
	...
	private LinkedList<AbstractWidget> displayModifiersFromTechnique() {
		...
		int currentTop = 2;
		for (var stat : this.modifier.getSavedStats()) {
			var value = this.modifier.getStat(stat);
			var label = new WuxiaLabel(2, currentTop, Component.translatable("wuxiacraft.gui." + stat.name().toLowerCase(), value), 0xFFFFFF);
			widgets.add(label);
			currentTop += 11;
		}
		for (var system : System.values()) {
			for (var stat : this.modifier.getSystemStats().get(system).keySet()) {
		...
		for (var stat : this.modifier.getElementalStats().keySet()) {
			for (var element : this.modifier.getElementalStats().get(stat).keySet()) {
				var value = this.modifier.getStat(stat, element);
				var label = new WuxiaLabel(2, currentTop, Component.translatable("wuxiacraft.gui." + stat.name().toLowerCase(), value), 0xFFFFFF);
				widgets.add(label);
				currentTop += 11;
			}
		}
		for (var system : this.modifier.getSystemElementalStats().keySet()) {
			for (var stat : this.modifier.getSystemElementalStats().get(system).keySet()) {
				for (var element : this.modifier.getSystemElementalStats().get(system).get(stat).keySet()) {
		...
		for (var element : this.modifier.elements.keySet()) {
			...
			var label = new WuxiaLabel(2, currentTop, Component.translatable(element.getNamespace() + ".element." + element.getPath())
					.append(Component.literal(": "))
					.append(Component.literal(String.format("%.1f", value))), 0xFFFFFF);
			widgets.add(label);
			currentTop += 11;
		}
		for (var weaponType : this.modifier.getWeaponStats().keySet()) {
			var value = this.modifier.getWeaponStats().get(weaponType);
			var label = new WuxiaLabel(2, currentTop, Component.translatable("wuxiacraft.weapon_type." + weaponType.name().toLowerCase())
					.append(Component.literal(": "))
					.append(Component.literal(String.format("%.1f", value))), 0xFFFFFF);
			widgets.add(label);
			currentTop += 11;
		...
		for (var skill : this.modifier.skills) {
			var label = new WuxiaLabel(2, currentTop, Component.translatable("wuxiacraft.gui.skill").append(Component.translatable(skill.getNamespace() + ".aspect." + skill.getPath())), 0xFFFFFF);
			widgets.add(label);
			currentTop += 11;
		}
		return widgets;
