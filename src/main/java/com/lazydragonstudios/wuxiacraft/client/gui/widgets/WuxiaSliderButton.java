...
public class WuxiaSliderButton extends AbstractSliderButton {
	...
	public WuxiaSliderButton(int pX, int pY, int pWidth, int pHeight, double pValue, Consumer<Double> apply) {
		super(pX, pY, pWidth, pHeight, Component.literal(String.format("%.1f%%", pValue * 100)), pValue);
		this.apply = apply;
	}

	@Override
	protected void updateMessage() {
		this.setMessage(Component.literal(String.format("%.1f%%", this.value * 100)));
	}
	...
	protected void onDrag(double pMouseX, double pMouseY, double pDragX, double pDragY) {
		if (!MathUtil.inBounds(pMouseX, pMouseY, this.getX(), this.getY(), this.getWidth(), this.getHeight())) return;
