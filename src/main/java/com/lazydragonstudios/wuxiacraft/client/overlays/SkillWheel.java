...
public class SkillWheel implements IGuiOverlay {

	private static final ResourceLocation SKILL_WHEEL = new ResourceLocation(WuxiaCraft.MOD_ID, "textures/gui/overlay/skill_wheel.png");
	...
	public void render(ForgeGui gui, GuiGraphics guiGraphics, float partialTicks, int width, int height) {
		...
		if (!InputHandler.KEY_SKILL_WHEEL.isDown() || !cultivation.isCombat()) {
			if (isShowingWheel) {
				sendSelectedAnother();
			}
		...
		guiGraphics.pose().translate(middleX, middleY, 0);
		...
		for (int i = 0; i < 10; i++) {
			...
			guiGraphics.blit(SKILL_WHEEL, -31, -105, 62, 72, texX, 0, 62, 72, 256, 256);
			...
			var zpRotation = new Quaternionf(new AxisAngle4f((float) Math.toRadians(-(i) * 36), 0, 0, 1));
			for (int j = 0; j < aspectCount; j++) {
				guiGraphics.pose().pushPose();
				double topOffset = 90 - j * divisions;
				guiGraphics.pose().translate(0, -topOffset, 0);
				guiGraphics.pose().mulPose(zpRotation);
				var aspect = skill.getSkillChain().get(j);
				var registryName = WuxiaRegistries.SKILL_ASPECT.get().getKey(aspect.getType());
				if (registryName == null) continue;
				guiGraphics.blit(new ResourceLocation(registryName.getNamespace(), "textures/skills/" + registryName.getPath() + ".png"), -16, -16, 32, 32, 0, 0, 32, 32, 32, 32);
				guiGraphics.pose().popPose();
			}
			guiGraphics.pose().pushPose();
			guiGraphics.pose().translate(0, -46, 0);
			guiGraphics.pose().mulPose(zpRotation);
			var numberString = String.format("%d", i + 1);
			var stringWidth = font.width(numberString);
			guiGraphics.drawString(gui.getFont(), numberString, -stringWidth / 2f, -font.lineHeight / 2f, 0xA0FFAA00, true);
			guiGraphics.pose().popPose();
			guiGraphics.pose().mulPose(new Quaternionf(new AxisAngle4f((float) Math.toRadians(36), 0, 0, 1)));
		}
		guiGraphics.pose().popPose();
