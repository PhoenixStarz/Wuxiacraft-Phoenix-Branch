...
public class KitsuneFiveTailedModel extends AbstractBodyTransformationModel {

	// This layer location should be baked with EntityRendererProvider.Context in the entity renderer and passed into this model's constructor
	public static final ModelLayerLocation LOCATION = new ModelLayerLocation(new ResourceLocation(WuxiaCraft.MOD_ID, "kitsune_5_tail"), "main");
	...
	public KitsuneFiveTailedModel(ModelPart root) {
	...
	public static LayerDefinition createMesh() {
		...
		PartDefinition tail_start_5 = tail_root.addOrReplaceChild("tail_start_5", CubeListBuilder.create().texOffs(0, 0).addBox(-1.5F, -1.5F, 0.0F, 3.0F, 3.0F, 4.0F, new CubeDeformation(0.0F)), PartPose.offsetAndRotation(0.0F, 0.0F, 2.0F, 0.0F, 0.0F, 0.0F));
	...
	public void setupAnim(AbstractClientPlayer pEntity, float limbSwing, float limbSwingAmount, float pAgeInTicks, float pNetHeadYaw, float pHeadPitch) {
		...
		pAgeInTicks += phaseIncrement;
		tail_start_5.xRot = 0.48f - Mth.abs(Mth.cos(limbSwing * 0.44F)) * 0.7F * limbSwingAmount;
		tail_start_5.yRot = Mth.cos(pAgeInTicks * 0.06F) * 0.1F + Mth.cos(limbSwing * 0.44F)*0.3f*limbSwingAmount;
		tail_middle_5.xRot = 0.15f - Mth.abs(Mth.cos(limbSwing * 0.26F)) * 0.4F * limbSwingAmount;
		tail_middle_5.yRot = Mth.cos(pAgeInTicks * 0.03F) * 0.1F;
		tail_end_5.xRot = -0.15f + Mth.cos(0.75f+limbSwing * 0.66F) * 0.7F * limbSwingAmount;
