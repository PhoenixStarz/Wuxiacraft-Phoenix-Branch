...
public class AnimatedPlayerRenderer extends PlayerRenderer {
	...
	public void render(AbstractClientPlayer player, float notKnowWhatThisIs, float partialTicks, PoseStack poseStack, MultiBufferSource multiBufferSource, int packedLightIn) {
		...
		if (!shouldSit && player.isAlive()) {
			limbSwingAmount = Mth.lerp(partialTicks, player.attackAnim, player.oAttackAnim);
			limbSwing = player.swingTime - player.oAttackAnim * (1.0F - partialTicks);
			if (player.isBaby()) {
				limbSwing *= 3.0F;
			}
		...
		if (!player.isSpectator()) {
			for (var renderLayer : this.layers) {
				if (renderLayer instanceof PlayerItemInHandLayer<AbstractClientPlayer, PlayerModel<AbstractClientPlayer>> && animData.isSwordFlight()) {
					poseStack.pushPose();
					poseStack.translate(-0.82D, 1.08D, 0.6D);
					poseStack.scale(1.2f, 1.2f, 1.2f);
					//poseStack.translate(0, -0.8, 0);
					poseStack.mulPose(new Quaternionf(new AxisAngle4f((float) Math.toRadians(9d), 0, 1, 0)));
					poseStack.mulPose(new Quaternionf(new AxisAngle4f((float) Math.toRadians(-80d), 1, 0, 0)));
					poseStack.mulPose(new Quaternionf(new AxisAngle4f((float) Math.toRadians(-90d), 0, 0, 1)));
		...
		net.minecraftforge.common.MinecraftForge.EVENT_BUS.post(new net.minecraftforge.client.event.RenderLivingEvent.Post<>(player, this, partialTicks, poseStack, multiBufferSource, packedLightIn));
		RenderNameTagEvent renderNameplateEvent = new RenderNameTagEvent(player, player.getDisplayName(), this, poseStack, multiBufferSource, packedLightIn, partialTicks);
	...
	private void setAnimations(AbstractClientPlayer player, PlayerModel<AbstractClientPlayer> model, float partialTicks) {
		...
		if (animationState.isMeditating()) {
			this.position.setComponent(1, -0.55f);
			...
			model.leftLeg.y = 12f;
			model.leftLeg.z = -1.8711f;
			model.rightLeg.x = -0.3299f;
			model.rightLeg.y = 12f;
			model.rightLeg.z = 1.8711f;
			model.leftLeg.xRot = (float) Math.toRadians(-9f);
			model.leftLeg.yRot = (float) Math.toRadians(30f);
			model.leftLeg.zRot = (float) Math.toRadians(0);
			model.rightLeg.xRot = (float) Math.toRadians(9f);
			model.rightLeg.yRot = (float) Math.toRadians(30f);
			model.rightLeg.zRot = (float) Math.toRadians(0);
			model.leftArm.x = 0.8682f;
			model.leftArm.y = 2f;
			model.leftArm.z = -4.9240f;
			model.rightArm.x = -0.8682f;
			model.rightArm.y = 2f;
			model.rightArm.z = 4.9240f;
			model.leftArm.xRot = 0;
			model.leftArm.yRot = (float) Math.toRadians(80.0f);
