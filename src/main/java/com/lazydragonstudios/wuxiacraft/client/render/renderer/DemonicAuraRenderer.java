...
public class DemonicAuraRenderer extends EntityRenderer<AbstractClientPlayer> {

	public static final ResourceLocation TEXTURE_LOCATION = new ResourceLocation(WuxiaCraft.MOD_ID, "textures/aura/demonic_aura.png");

	private Aura aura;

	public DemonicAuraRenderer(EntityRendererProvider.Context pContext) {
		super(pContext);
		aura = new ElectricAura();
	}
	...
	public void render(AbstractClientPlayer player, float notKnown, float partialTicks, PoseStack poseStack, MultiBufferSource bufferSource, int packedLight) {
		...
		if (this.aura instanceof ElectricAura eAura) {
			eAura.renderSpecial(poseStack, bufferSource, packedLight, overlay, 0.4f, 0.01f, 0.01f, 1f);
		}
		poseStack.popPose();
	}
