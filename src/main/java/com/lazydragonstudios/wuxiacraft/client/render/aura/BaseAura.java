...
public class BaseAura extends Aura {
	...
	public void render(PoseStack poseStack, VertexConsumer vertexConsumer, int packedLightIn, int packedOverlayIn, float red, float green, float blue, float alpha) {
		...
		for (var strand : this.strands) {
			...
			poseNormals.transform(strandNormals);
			...
			for (int i = 0; i < 4; i++) {
				...
				poseStack.last().pose().transform(vertexPosInWorld);
	...
	public void update() {
		...
		float maxW = 1.1f;
		for (var strand : this.strands) {
			strand.setComponent(3, strand.w() + raiseSpeed);
			if (strand.w() > maxW) {
				var newStrand = createNewStrand();
				strand.setComponent(0, newStrand.x());
				strand.setComponent(1, newStrand.y());
				strand.setComponent(2, newStrand.z());
				strand.setComponent(3, newStrand.w());
			}
		}
	}
}
