...
public class KitsuneThreeTailedModel extends AbstractBodyTransformationModel {

	// This layer location should be baked with EntityRendererProvider.Context in the entity renderer and passed into this model's constructor
	public static final ModelLayerLocation LOCATION = new ModelLayerLocation(new ResourceLocation(WuxiaCraft.MOD_ID, "kitsune_3_tail"), "main");
	...
	public KitsuneThreeTailedModel(ModelPart root) {
	...
	public static LayerDefinition createMesh() {
		...
		PartDefinition tail_start_3 = tail_root.addOrReplaceChild("tail_start_3", CubeListBuilder.create().texOffs(0, 0).addBox(-1.5F, -1.5F, 0.0F, 3.0F, 3.0F, 4.0F, new CubeDeformation(0.0F)), PartPose.offsetAndRotation(0.0F, 0.0F, 2.0F, 0.2618F, 0.0F, 0.0F));
	...
	public void setupAnim(AbstractClientPlayer pEntity, float limbSwing, float limbSwingAmount, float pAgeInTicks, float pNetHeadYaw, float pHeadPitch) {
		...
		tail_start_1.xRot = -0.2618F - Mth.abs(Mth.cos(limbSwing * 0.44F)) * 0.7F * limbSwingAmount;
		...
		pAgeInTicks += phaseIncrement;
		tail_start_3.xRot = 0.2618F + 0.48f - Mth.abs(Mth.cos(limbSwing * 0.44F)) * 0.7F * limbSwingAmount;
		tail_start_3.yRot = Mth.cos(pAgeInTicks * 0.03F) * 0.3F + Mth.cos(limbSwing * 0.44F) * 0.3f * limbSwingAmount;
