...
public class SkillSwordFlightActivator extends SkillActivatorAspect {
	...
	public SkillSwordFlightActivator() {
		this.activate = (player, skill) -> {
			...
			var max_speed = cultivation.getStat(PlayerStat.AGILITY, true).multiply(skill.getStatValue(SkillStat.STRENGTH)).multiply(new BigDecimal("0.1"));
			var speed = max_speed.multiply(BigDecimal.valueOf(cultivation.getAgilityRegulator()));
			var cost = skill.getStatValue(SkillStat.COST);
			//cost = (cost*0.5)*(1+agility_regulator) --> had to add this or ppl could fly for free if agility was low
			cost = cost.multiply(new BigDecimal("0.5")).multiply(BigDecimal.ONE.add(BigDecimal.valueOf(cultivation.getAgilityRegulator())));
			//consumed energy = cost*(agilityRegulator)
