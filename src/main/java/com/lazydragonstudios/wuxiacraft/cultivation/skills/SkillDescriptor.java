...
public class SkillDescriptor {
		...
		if (tag.contains("skills-list")) {
			...
			if (skillsTag instanceof ListTag skillsList) {
				for (var rawSkill : skillsList) {
					...
					var skillAspect = SkillAspect.readAspect(skillTag, cultivation);
	...
	/**
	 * This will get the compiled stats and multiply by the essence strength.
	 * Useful to calculate total damage or total cost from a skill
	 * This is usually a way to apply skill parameters
	 * @param stat the stats to be gotten
	 * @return the stat value
	 */
	public BigDecimal getAppliedStats(ICultivation cultivation, SkillStat stat) {
		var baseStat = this.getStatValue(stat);
