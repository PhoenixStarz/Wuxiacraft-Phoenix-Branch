...
import com.lazydragonstudios.wuxiacraft.cultivation.technique.aspects.SystemGather;
...
public class TechniqueGrid {
	...
	public TechniqueModifier compile() {
		...
		var paths = new LinkedList<LinkedList<Point>>();
		getPathsToGather(startNodePoint, processHierarchy, new LinkedList<>(), paths);
		TechniqueModifier tMod = getModifiersFromMetaData(metaData);
		for (var path : paths) {
			var pathMetaData = new HashMap<String, Object>();
			for (var visiting : path) {
				var aspect = WuxiaRegistries.TECHNIQUE_ASPECT.get().getValue(this.grid.get(visiting));
				var proficiency = this.proficiencies.getOrDefault(visiting, BigDecimal.ZERO);
				if (aspect == null) continue;
				aspect.accept(pathMetaData, proficiency);
			}
			tMod.add(getModifiersFromMetaData(pathMetaData));
		}
		return tMod;
	}

	private void getPathsToGather(Point visiting, HashMap<Point, HashSet<Point>> processHierarchy, LinkedList<Point> visitingPath, LinkedList<LinkedList<Point>> correctPaths) {
		if (visitingPath.contains(visiting)) return;
		visitingPath.add(visiting);
		if (processHierarchy.get(visiting).isEmpty()) {
			var aspectLocationAtEnd = this.grid.get(visiting);
			var aspect = WuxiaRegistries.TECHNIQUE_ASPECT.get().getValue(aspectLocationAtEnd);
			if (aspect instanceof SystemGather) {
				correctPaths.add(visitingPath);
			}
		}
		for (var nextPoint : processHierarchy.get(visiting)) {
			//noinspection unchecked
			getPathsToGather(nextPoint, processHierarchy, (LinkedList<Point>) visitingPath.clone(), correctPaths);
		}
	}
	...
	public static void traverseGridFromStart(TechniqueGrid grid, Consumer<Point> visit, BiConsumer<Point, Point> onConnect, BiConsumer<Point, Point> onJunked, Consumer<Point> onDisconnected) {
		...
		HashMap<Point, LinkedList<TechniqueAspect>> connectedToAspects = new HashMap<>();
		...
		while (!toVisit.isEmpty()) { // layer wide iteration aka Breadth first
			...
			connectedToAspects.put(visiting, new LinkedList<>());
			connectedFrom.putIfAbsent(visiting, new HashSet<>());
...
//				if (visited.contains(visitingNeighbour)) continue;
			...
			for (var candidate : connectCandidates) {
				...
				if (connectedFrom.get(visiting).contains(candidate)) continue;
				if ((allowedFrom == -1 || cFrom < allowedFrom) && (allowedTo == -1 || cTo < allowedTo)
						&& aspect.shouldConnect(candidateTechAspect, connectedToAspects.get(visiting))) {
					connectedFrom.get(candidate).add(visiting);
					connectedTo.get(visiting).add(candidate);
					connectedToAspects.get(visiting).add(candidateTechAspect);
					toVisit.add(candidate);
					onConnect.accept(visiting, candidate);
					if (junkNotExpected.containsKey(candidate)) {
						junkNotExpected.remove(candidate);
					}
	...
	private static TechniqueModifier getModifiersFromMetaData(HashMap<String, Object> metaData) {
		...
		for (var stat : PlayerStat.values()) {
			if (metaData.containsKey("stat-" + stat.name().toLowerCase())) {
				tMod.setStat(stat, (BigDecimal) metaData.get("stat-" + stat.name().toLowerCase()));
			}
		}
		for (var stat : PlayerElementalStat.values()) {
			for (var element : WuxiaRegistries.ELEMENTS.get().getKeys()) {
				var statName = element.toString().toLowerCase() + "-stat-" + stat.name().toLowerCase();
				if (metaData.containsKey(statName)) {
					tMod.setStat(stat, element, (BigDecimal) metaData.get(statName));
				}
			}
		}
		for (var system : System.values()) {
			for (var stat : PlayerSystemStat.values()) {
				...
				if (metaData.containsKey(statName)) {
					tMod.getSystemStats().get(system).put(stat, ((BigDecimal) metaData.get(statName)).max(BigDecimal.ZERO).setScale(4, RoundingMode.HALF_DOWN));
			...
			for (var stat : PlayerSystemElementalStat.values()) {
				for (var element : WuxiaRegistries.ELEMENTS.get().getKeys()) {
					String statName = system.name().toLowerCase() + "-" + element.toString().toLowerCase() + "-stat-" + stat.name().toLowerCase();
					if (metaData.containsKey(statName)) {
						tMod.setStat(system, stat, element, ((BigDecimal) metaData.get(statName)).max(BigDecimal.ZERO));
					}
				}
			}
		}
		for (var element : WuxiaRegistries.ELEMENTS.get().getValues()) {
			if (metaData.containsKey("element-" + element.getName())) {
				tMod.elements.put(WuxiaRegistries.ELEMENTS.get().getKey(element), (Double) metaData.get("element-" + element.getName()));
			}
		}
		for (var weaponType : WeaponElementalGenerator.WeaponType.values()) {
			if (metaData.containsKey("weapon-" + weaponType.name().toLowerCase())) {
				tMod.getWeaponStats().put(weaponType, (BigDecimal) metaData.getOrDefault("weapon-" + weaponType.name().toLowerCase(), BigDecimal.ZERO));
			}
		}
