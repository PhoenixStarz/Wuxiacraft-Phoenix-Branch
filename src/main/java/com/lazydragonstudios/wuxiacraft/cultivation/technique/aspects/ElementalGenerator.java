...
public class ElementalGenerator extends TechniqueAspect {
	...
	public boolean canConnect(TechniqueAspect aspect) {
		...
		if (aspect instanceof DualElementalGenerator gen) {
			return (gen.element1.equals(this.element)
				 || gen.element2.equals(this.element));
		}
		if (aspect instanceof TriElementalGenerator gen) {
			return (gen.element1.equals(this.element)
				 || gen.element2.equals(this.element)
				 || gen.element3.equals(this.element));
		}
		if (aspect instanceof QuadElementalGenerator gen) {
			return (gen.element1.equals(this.element)
				 || gen.element2.equals(this.element)
				 || gen.element3.equals(this.element)
				 || gen.element4.equals(this.element));
		}
		if (aspect instanceof QuinElementalGenerator gen) {
			return (gen.element1.equals(this.element)
				 ...
				 || gen.element3.equals(this.element)
				 || gen.element4.equals(this.element)
				 || gen.element5.equals(this.element));
		}
	...
	public void renderTooltip(PoseStack poseStack, int mouseX, int mouseY) {
		...
		var tooTipWidth = Math.max(nameWidth + 10, 30 + amountWidth);
	...
	public LinkedList<AbstractWidget> getStatsSheetDescriptor(BigDecimal proficiency) {
		...
		widgets.add(4, new WuxiaLabel(0, 0, new TranslatableComponent("wuxiacraft.gui.generates", generated + " " + element.getPath()), 0xFFAA00));
		return widgets;
	}
