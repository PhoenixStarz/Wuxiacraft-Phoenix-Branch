...
public class WuxiaTechniqueAspects {
	...
	() -> new ConditionalElementalGenerator(3d, WuxiaElements.PHYSICAL.getId()) {
		...
		public void onCultivate(CultivatingEvent event) {
			...
			for (var element : devourData.keySet()) {
				baseAmount = baseAmount.add(devourData.get(element));
				systemData.addStat(element, PlayerSystemElementalStat.FOUNDATION, baseAmount.multiply(new BigDecimal("0.3")));
	...
	public static RegistryObject<TechniqueAspect> ARTIFACT_OF_FLAME = ASPECTS.register("artifact_of_flame",
			() -> new TechniqueAspect() 			
			{
				@Override
			...
			.addCheckpoint(new TechniqueAspect.Checkpoint("artifact", new BigDecimal("1"), new BigDecimal("0"),
			cultivation -> cultivation.getAspects().learnAspect(new ResourceLocation(WuxiaCraft.MOD_ID, "divine_flame"), cultivation)))
			.addCheckpoint(new TechniqueAspect.Checkpoint("artifact", new BigDecimal("1"), new BigDecimal("0"),
			cultivation -> cultivation.getAspects().learnAspect(new ResourceLocation(WuxiaCraft.MOD_ID, "divine_immolation"), cultivation)))
	);
	public static RegistryObject<TechniqueAspect> DIVINE_FLAME = ASPECTS.register("divine_flame",
			() -> new ElementalGenerator(19683d, WuxiaElements.FIRE.getId())
					.setCanLearn(cultivation -> cultivation.getAspects()
							.knowsAspect(new ResourceLocation(WuxiaCraft.MOD_ID, "artifact_of_flame")))
							.addCheckpoint(new TechniqueAspect.Checkpoint("artifact", new BigDecimal("1"), new BigDecimal("0")))
	);
	public static RegistryObject<TechniqueAspect> DIVINE_IMMOLATION = ASPECTS.register("divine_immolation",
			() -> new ElementSystemConverter(360d, WuxiaElements.FIRE.getId(), System.ESSENCE)
					.setCanLearn(cultivation -> cultivation.getAspects()
							.knowsAspect(new ResourceLocation(WuxiaCraft.MOD_ID, "artifact_of_flame")))
							.addCheckpoint(new TechniqueAspect.Checkpoint("artifact", new BigDecimal("1"), new BigDecimal("0")))
	);
