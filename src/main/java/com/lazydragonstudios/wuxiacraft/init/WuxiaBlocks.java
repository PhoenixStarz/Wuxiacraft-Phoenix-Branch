...
public class WuxiaBlocks {
	...
	public static RegistryObject<Block> CELESTIAL_IRON_ORE = BLOCKS.register("celestial_iron_ore",
			() -> new Block(BlockBehaviour.Properties.of(Material.STONE).strength(8f).lightLevel(s -> 3).requiresCorrectToolForDrops()));
	...
	public static RegistryObject<Block> CELESTIAL_IRON_BLOCK = BLOCKS.register("celestial_iron_block",
			() -> new Block(BlockBehaviour.Properties.of(Material.METAL).strength(8f).lightLevel(s -> 3).requiresCorrectToolForDrops()));
	...
	public static RegistryObject<Block> DIVINE_FLAME = BLOCKS.register("divine_flame",
		...
		for (var material : FormationMaterialTier.values()) {
			...
			BARRIER_RUNES.put(material, BLOCKS.register(material.name().toLowerCase() + "_barrier_rune",
					() -> new StatRuneBlock(BlockBehaviour.Properties.of(material.blockMaterial).strength(material.blockStrength))
							...
							.addStat(FormationStat.BARRIER_RANGE, new BigDecimal("5").add(material.materialModifier.multiply(new BigDecimal("1"))))
			...
			for (var system : System.values()) {
				...
				ENERGY_RUNES.get(system).put(material, BLOCKS.register(system.name().toLowerCase() + "_" + material.name().toLowerCase() + "_energy_rune",
						() -> new StatRuneBlock(BlockBehaviour.Properties.of(material.blockMaterial).strength(material.blockStrength))
								...
								.addStat(system, FormationSystemStat.ENERGY_REGEN, material.materialModifier.multiply(new BigDecimal("0.0025")))
				...
				CULTIVATION_RUNES.get(system).put(material, BLOCKS.register(system.name().toLowerCase() + "_" + material.name().toLowerCase() + "_cultivation_rune",
						() -> new StatRuneBlock(BlockBehaviour.Properties.of(material.blockMaterial).strength(material.blockStrength))
								...
								.addStat(system, FormationSystemStat.CULTIVATION_SPEED, material.materialModifier.multiply(new BigDecimal("0.1")))
