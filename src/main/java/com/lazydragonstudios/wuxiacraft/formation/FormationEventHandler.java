		...
		for (var system : System.values()) {
			...
			if (distToFormation <= regenDist * regenDist) {
				if (energyRegen.compareTo(new BigDecimal("0.005")) >= 0 &&
						//currentEnergy + regenAmount <= maxEnergy
						currentEnergy.add(regenAmount).compareTo(maxEnergy) <= 0) {
					systemData.addEnergy(formationRegenAmount);
				} else 	if (currentEnergy.add(regenAmount).compareTo(maxEnergy) <= 0) {
					systemData.addEnergy((formationRegenAmount).multiply(new BigDecimal("0.2")));
				}
				if (currentEnergy.add(regenAmount).compareTo(maxEnergy.multiply(new BigDecimal("1.1"))) > 0) {
					systemData.addEnergy((formationRegenAmount).multiply(new BigDecimal("-1")));
				}
			}
		}
